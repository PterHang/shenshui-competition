...
class Encoder(Model):
    def __init__(self):
        super().__init__()
        self.lstm = layers.LSTM(units=64, input_shape=(time_steps, n_features), return_sequences=True, return_state=True)
...
class BiLSTM(Model):
    def __init__(self):
        super().__init__()
        # self.bilstm = layers.Bidirectional(layers.LSTM(128, input_shape=(time_steps, n_features), return_sequences=True))
        self.bilstm = layers.LSTM(128, input_shape=(time_steps, n_features), return_sequences=True)
